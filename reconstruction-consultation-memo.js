(() => {
  ...
  async function build(options={}){
    ...
    const entries=await Promise.all(categories.map(async category=>{
      ...
      const guidance=window.ReconstructionActionNav?.getGuidance?await window.ReconstructionActionNav.getGuidance(category,municipalityId):null;
      return {category,actions:actions.slice(0,1).map(action=>({title:action.title,warning:action.kind==="program_specific"&&(action.highRisk||action.doNotDoYet),doNotDoYet:action.doNotDoYet||""})),guidance,official:officialFor(municipality,category)};
    }));
  ...
  function markup(memo){
    ...
    const guidance=memo.entries.map(entry=>entry.guidance).filter(Boolean);const contactRows=(key,label)=>guidance.flatMap(item=>item.contacts?.[key]||[]).slice(0,2).map(contact=>`<li><strong>${label}：${clean(contact.name)}</strong><span>発表主体：${clean(contact.organization)}</span>${contact.phone?`<span>電話：${clean(contact.phone)}</span>`:""}${contact.hours?`<span>受付時間：${clean(contact.hours)}</span>`:""}</li>`).join("");const contactGuidance=[contactRows("application","申請するところ"),contactRows("inquiry","制度について聞くところ"),contactRows("consultation","個別の状況を相談するところ")].join("");const documentGuidance=guidance.flatMap(item=>item.documents||[]).filter((item,index,all)=>all.findIndex(other=>other.id===item.id)===index).slice(0,3).map(document=>`<li><strong>${clean(document.name)}</strong><span>${clean(document.description||document.requiredLabel)}</span><small>${clean(document.scopeLabel)}</small></li>`).join("");const guidanceSection=contactGuidance||documentGuidance?`<section><h2>申請先・相談先・主な必要書類</h2><p class="memo-section-note">確認済み情報だけを簡潔に載せています。このメモだけで申請が完了するとは限りません。</p>${contactGuidance?`<ul class="memo-officials">${contactGuidance}</ul>`:""}${documentGuidance?`<h3>主な必要書類</h3><ul class="memo-officials">${documentGuidance}</ul>`:""}</section>`:"";
    return `<div class="memo-screen-actions" aria-label="確認メモの操作"><button type="button" data-close-consultation-memo>暮らし整理ナビへ戻る</button><button type="button" data-print-consultation-memo>確認メモを印刷</button></div><article class="memo-sheet"><header><p>暮らしの再建</p><h1 id="consultation-memo-title">確認メモ</h1><p class="memo-purpose">今、何を確認したいかを本人・家族・支援者と一緒に見るための簡単な確認メモです。診断結果、申請書、行政の公式書類ではありません。</p></header><dl class="memo-meta"><div><dt>対象地域</dt><dd>${clean(memo.municipalityName)}</dd></div><div><dt>このメモを作成</dt><dd>${clean(memo.createdAt)}</dd></div></dl><section><h2>選んだ困りごと</h2><ul class="memo-categories">${categories}</ul></section><section><h2>次に確認すること</h2><p class="memo-section-note">表示順に優先順位の意味はありません。公式情報を一緒に確認するための案内です。</p><ul class="memo-actions">${actionMarkup(actions)}</ul></section>${guidanceSection}<section><h2>参考にする公式情報</h2><ul class="memo-officials">${officials||'<li>自治体の災害関連公式ページをご確認ください。該当制度がないという意味ではありません。</li>'}</ul></section><section class="memo-handwriting"><h2>気になること・確認したいこと</h2><div aria-hidden="true"></div><div aria-hidden="true"></div></section><footer><p><strong>情報更新の注意：</strong>災害情報や制度情報は更新されることがあります。最新情報は公式ページで再確認してください。</p><p>一般社団法人よか隊ネット熊本が、自治体等の公式情報を探しやすく整理したページから作成した確認メモです。</p><p class="memo-privacy">選んだ内容はサーバーに保存されません。</p></footer></article>`;
  }
