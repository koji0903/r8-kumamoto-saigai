(()=>{
  ...
  const facilities=[
    ...
    {id:"kireka",name:"環境再生センターKIREKA",address:"宇土市松原町386",lat:32.696476,lng:130.658844,ticket:true,closed:"8月18日以降：毎週火曜・土曜",types:["burnable","fabric","wood","metal","sofa","small","building","appliance"],note:"可燃ごみ（生活ごみを除く）、プラスチック類、布団・布類、木くず、金属類、ソファー・マット、小型家電、瓦、コンクリート殻、廃家電、陶磁器、ガラス、スレート板、サイディングボード、石膏ボード",flow:"施設へ直接並べません。三菱ケミカル駐車場で入場券を受け取り、誘導に従ってください。"},
  ...
  const render=()=>{
    ...
    results.innerHTML=`<header class="facility-result-heading"><span class="result-step">2</span><div><p>選択した品目：<b>${current.label}</b></p><h3>持ち込める場所はこちら</h3><span>${found.length}施設が該当します</span></div><i aria-hidden="true">↓</i></header>`+found.map(f=>`<article class="facility-card"><header><span class="facility-number">${facilities.indexOf(f)+1}</span><div><p>${f.ticket?"入場券が必要":"直接搬入"}</p><h3>${f.name}</h3><span>${f.address}</span></div></header><div class="facility-status ${f.ticket?"needs-ticket":"direct"}">${f.ticket?icon('<path d="M4 7h16v4a2 2 0 0 0 0 4v4H4v-4a2 2 0 0 0 0-4V7Z"/>'):icon('<path d="M3 11h18M5 11l2-5h10l2 5v7H5z"/>')}<b>${f.ticket?"入場券を受け取ってから搬入":"この施設へ直接搬入"}</b></div><dl><div><dt>受付</dt><dd>8:30〜11:30／13:00〜15:30</dd></div><div><dt>停止日</dt><dd>${f.closed}</dd></div></dl><div class="accepted"><header><span aria-hidden="true">${icon('<path d="M4 7h16v13H4zM8 7V4h8v3M9 11v5m6-5v5"/>')}</span><div><b>持ち込めるごみ</b><small>${f.id==="kanemura"?"この3種類だけです":"8分類・15品目を受け入れます"}</small></div></header><div class="accepted-list">${f.types.map(key=>{const c=categories.find(x=>x.key===key);return `<span><i>${c.art}</i><b>${c.label}</b></span>`}).join("")}</div><details><summary>正式な品目を確認する</summary><p>${f.note}</p></details></div><p class="facility-flow">${f.flow}</p><a class="route-link" href="${mapsUrl(f)}" target="_blank" rel="noopener">現在地から経路を検索 ↗</a></article>`).join("");
