(()=>{
  ...
  const render=()=>{
    ...
    results.innerHTML=`<header class="facility-result-heading"><span class="result-step">2</span><div><p>選択した品目：<b>${current.label}</b></p><h3>持ち込める場所はこちら</h3><span>${found.length}施設が該当します</span></div><i aria-hidden="true">↓</i></header>`+found.map(f=>`<article class="facility-card"><header><span class="facility-number">${facilities.indexOf(f)+1}</span><div><p>${f.ticket?"入場券が必要":"直接搬入"}</p><h3>${f.name}</h3><span>${f.address}</span></div></header><div class="facility-status ${f.ticket?"needs-ticket":"direct"}">${f.ticket?icon('<path d="M4 7h16v4a2 2 0 0 0 0 4v4H4v-4a2 2 0 0 0 0-4V7Z"/>'):icon('<path d="M3 11h18M5 11l2-5h10l2 5v7H5z"/>')}<b>${f.ticket?"入場券を受け取ってから搬入":"この施設へ直接搬入"}</b></div><dl><div><dt>受付</dt><dd>8:30〜11:30／13:00〜15:30</dd></div><div><dt>停止日</dt><dd>${f.closed}</dd></div></dl><div class="accepted"><b>受け入れるもの</b><p>${f.note}</p></div><p class="facility-flow">${f.flow}</p><a class="route-link" href="${mapsUrl(f)}" target="_blank" rel="noopener">現在地から経路を検索 ↗</a></article>`).join("");
