...
for(const m of result.municipalities){assert.ok(m.municipalityId&&m.officialUrl);const seen=new Set();for(const u of m.updates){assert.ok(u.title===undefined&&u.originalTitle&&u.displayTitle);assert.ok(u.categories.length);assert.equal(seen.has(u.url),false);seen.add(u.url);assert.ok(new URL(u.url).hostname.endsWith(new URL(m.officialUrl).hostname));}}
