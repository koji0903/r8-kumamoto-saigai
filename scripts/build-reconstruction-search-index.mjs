...
for(const program of programs){if(program.availability?.state!=="confirmed")continue;const guidanceKeywords=[...(program.documents||[]).map(item=>item.name),...(program.municipalities||[]).flatMap(item=>[...Object.values(item.contacts||{}).flat().map(contact=>contact.name),...(item.documents||[]).map(document=>document.name)])];for(const source of program.officialSources||[])if(/^https:\/\//.test(source.url||""))records.push({id:`program:${program.id}:${source.url}`,title:program.title,organizationName:source.organization,governmentLevel:program.governmentLevel==="national"?"国":program.governmentLevel==="prefecture"?"熊本県":"公的機関",municipalityId:null,municipalityName:null,categories:program.categories||[],keywords:[program.officialName,program.benefitType,...guidanceKeywords].filter(Boolean),summary:program.summary,publishedAt:null,updatedAt:null,retrievedAt:null,url:source.url,sourceType:"verified_program",publicationStatus:"published",verificationStatus:"verified",freshnessStatus:"current"});}
