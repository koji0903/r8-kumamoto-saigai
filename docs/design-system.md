...
- 制度、用語、行動、対象者などを並べる説明カードでは、内容を直感的に表すアイコンを必ず併用する
- 漢字一文字、数字、頭文字をアイコンの代用品として使わない。家、書類、契約、修理、相談など、意味に対応する具体的な図形を使う
- 同一カード群ではアイコンのサイズ、線幅、背景面、配置を統一する
- 装飾目的で増やさず、見出しを読む前でも分類や行動の違いを予測できる図形を選ぶ
